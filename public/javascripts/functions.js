...
/**
 * @author segulee
 * @description login 세션을 가지고 현재 이름이 포함된 메인을 보여주는 페이지에 접근합니다.
 *              해당 페이지의 html 소스를 반환합니다.
 * @returns {*|Promise}
 */
exports.getNameMain = function () {
    return new Promise(function (resolve, reject){
        request({
...
exports.getName = function (getNameMain) {
    return new Promise(function (resolve, reject) {
        let $ = cheerio.load(getNameMain);
        let name = $('.stats_login_sec > strong');
        if(!name){
            reject({code:'LOGIN_ERR', message: '  로그인 제대로 안됨!'});
        } else {
    ...
    return new Promise(function (resolve, reject) {
        ...
        tableTrs.each(function (i) {
            if (i % 7 === 1) {
                ...
                console.log(lectureLinkList[count++].lectureName);
            }
        });
